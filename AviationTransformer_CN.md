...
[背景] 传统的航空气象服务严重依赖高度编码的字母数字格式及碎片化的非结构化文本，随着通用航空需求的激增，飞行员在解码与整合过载气象数据时的认知负荷已成为制约决策效率的关键瓶颈。 [问题] 尽管大语言模型（LLM）为气象信息的智能化解析提供了新途径，但其在垂直领域的“幻觉”问题、对数值信息的处理缺陷，以及高性能模型在驾驶舱环境下的本地部署受限，均为系统的准确性与稳定性带来了挑战。 [方法] 针对上述问题，本文提出了一种融合知识图谱与小参数模型的智能处理框架。首先，构建航空气象领域的知识图谱（Graph-RAG），并结合NTSB事故数据与理想气象模型，建立了气象危险度评估数据库。其次，采用 RALA-DPO（Retrieval-Augmented Logic Awareness Direct Preference Optimization，注：此处假设RALA是你方法的缩写，若不是请修正）对 Qwen3-8B 轻量级模型进行微调，构建了一个集航路气象信息整理、增强及自然语言转述于一体的智能系统。 [结果] 实验结果表明，在 Graph-RAG 的辅助下，该微调模型在气象风险评估与信息转述任务上的准确率较基准模型（Baseline）提升了 21%。 [结论] 这一显著提升证实，即便在航空气象这样高度专业化且资源受限的场景中，结合特定领域知识增强的 RALA-DPO 方法仍能有效释放小参数模型的潜力，为驾驶舱智能辅助提供了可行的解决方案。
