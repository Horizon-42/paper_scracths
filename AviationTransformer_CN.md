...
(Context)
传统的航空气象服务依赖高度编码的字母数字格式以及碎片化的非结构化文本传输信息,随着通用航空需求的增长, 解码、整合过载的气象数据已经成为制约飞行员决策效率的关键瓶颈,
(Problem)
随着大模型的兴起, 构建智能化的气象信息解析、转述系统成为可能. 但是大模型在垂直专业领域的幻觉及其对数值信息碎片化处理方式为提高系统的准确性及稳定性提出了挑战. 在飞行驾驶舱环境, 高性能大模型的本地部署本身也困难重重.
(Method) 
为了解决上述问题, 本文构建了一个航空气象领域的知识图谱数据库(Graph RAG), 并应用教师-学生方法对小参数模型进行了微调, 构建了一个可以将航路气象信息整理, 增强, 并转述为自然语言叙述的智能信息处理系统.
(Results) 
(Conclusion) 
